...
- Shared CONFIG: same-name fields share ALL configuration, not just the value. `useFieldStore.updateField`/`updateFields` propagate every non-positional prop to same-name siblings; `PER_WIDGET_KEYS` (`id`,`name`,`page`,`x`,`y`,`width`,`height`) are the ONLY keys that stay per-widget (each widget keeps its own placement; `name` is the group key). Renaming a field into an existing same-name group adopts that group's full config (`shareableConfig`). So font/size/align/bold/value/etc. stay consistent across the shared group on canvas, filler and export.
