...
import { canvasToPdf, pdfToCanvas } from '../../src/utils/coordinates';
...
describe('pdfToCanvas', () => {
  test('maps PDF bottom-left to canvas top-left correctly', () => {
    // Field at PDF (72, 680) with size (200×24) on a 792pt-tall page
    const result = pdfToCanvas(72, 680, 200, 24, SCALE, PAGE_HEIGHT);
    // top = (792 - 680 - 24) * 1.5 = 88 * 1.5 = 132
...
describe('canvasToPdf', () => {
  test('maps canvas top-left to PDF bottom-left correctly', () => {
    const canvasX = 108;  // 72 * 1.5
    const canvasY = 132;  // (792 - 680 - 24) * 1.5
    const widthPx = 300;  // 200 * 1.5
    const heightPx = 36;  // 24 * 1.5
