...
import { TemplatePanel } from '../../src/components/TemplatePanel/TemplatePanel';
import type { Template } from '../../src/hooks/useTemplateStore';
...
describe('TemplatePanel', () => {
  ...
  it('(2) save button is disabled when name is longer than 100 chars', () => {
    ...
    // maxLength=100 on the input prevents entering >100 chars natively, so input stays ≤100
    // In jsdom, maxLength is not enforced on fireEvent.change, but we test the disabled logic
    // Use a name that React sees as > 100 after trim
    const saveBtn = getByRole('button', { name: 'Guardar' });
    // With maxLength=100 the input value is capped by the browser; in jsdom it isn't,
    // so test both paths:
    // If input.value.trim().length > 100 → disabled
  ...
  it('(7) Renombrar switches to inline edit; Enter confirms, Escape cancels', () => {
    ...
    // Start rename again and confirm with Enter
    fireEvent.click(getAllByRole('button', { name: 'Renombrar' })[0]);
